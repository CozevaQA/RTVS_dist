...
def num_den_comparer(num_den_value_list, ws, wb, path1, filename):
    ...
    for i in range(0, len(num_den_value_list), 8):
        try:
            if 0.00 <= (num_den_value_list[i] - num_den_value_list[i + 4]) / num_den_value_list[i] > 0.1 and 0.00 <= (
                    num_den_value_list[i + 1] - num_den_value_list[i + 5]) / num_den_value_list[i + 1] > 0.1:
                ws.append([customer_name, num_den_value_list[i + 2], num_den_value_list[i + 3],
                           ...
                           "Issue: RCC V24 Score is less than RCC Blended Score", "-"])
                ...
                ws.append([customer_name, num_den_value_list[i + 2], num_den_value_list[i + 3],
                           ...
                           "RCC V24 Score is greater than RCC Blended Score as expected", "-"])
            else:
                print("Score section skipped")
                    ...
                    if (num_den_value_list[i + 1] - num_den_value_list[i + 5]) / num_den_value_list[i + 5] < 0.11:
                        ...
                        ws.append([customer_name, num_den_value_list[i + 2], num_den_value_list[i + 3],
                                   "RCC(Blended) score: " + str(num_den_value_list[i]) + "/" + str(
                                       ...
                                       num_den_value_list[i + 4]) + "/" + str(
                                       num_den_value_list[i + 5]),
                                   "RCC v24(ID:33) scores are greater than RCC- Blended(ID:400) score [Expected for MY2024]", "-"])
                except ZeroDivisionError as e:
                    ws.append([customer_name, num_den_value_list[i + 2], num_den_value_list[i + 3],
                               "RCC(Blended) score: " + str(num_den_value_list[i]) + "/" + str(
                                   num_den_value_list[i + 1]) + "|| RCC(V24) score: " + str(
                ...
                if len(table) == 1 and "No data" in table[0].text:
                    ...
                    while pat_count != 0:
                        ...
                        if selected_patient.find_element(By.CLASS_NAME, "risk_score_gap ").text == "NA":
                            ...
                            # #This portion of the code changes the dropdown to V28 or V24
                            # WebDriverWait(driver, 120).until(EC.presence_of_element_located(
                            #     (By.XPATH, "//div[contains(@class, 'select-wrapper hcc-model-menu')]")))
                            # if "Version 24" in Measure:
                            #     print("Continuing with V24 Version")
                            #     driver.find_element(By.XPATH,
                            #                         "//div[contains(@class, 'select-wrapper hcc-model-menu')]").click()
                            #     dropdown_opts = driver.find_element(By.XPATH,
                            #                                         "//div[contains(@class, 'select-wrapper hcc-model-menu')]").find_element(
                            #         By.TAG_NAME, "ul").find_elements(By.TAG_NAME, "li")
                            #     print(dropdown_opts)
                            #     for opt in dropdown_opts:
                            #         if opt.text == "CMS-HCC V24":
                            #             opt.click()
                            #             time.sleep(2)
                            #             break
                            # elif "Version 28" in Measure:
                            #     print("Continuing with V28 Version")
                            #     driver.find_element(By.XPATH,
                            #                         "//div[contains(@class, 'select-wrapper hcc-model-menu')]").click()
                            #     dropdown_opts = driver.find_element(By.XPATH,
                            #                                         "//div[contains(@class, 'select-wrapper hcc-model-menu')]").find_element(
                            #         By.TAG_NAME, "ul").find_elements(By.TAG_NAME, "li")
                            #     for opt in dropdown_opts:
                            #         if opt.text == "CMS-HCC V28":
                            #             opt.click()
                            #             break
                            # else:
                            #     print("Continuing with Blended Version")
...
def MSPLScoreCheck(driver,ws, wb, LOB_Name, Measure):
    ...
    list1 = ["HCC Score (Blended)", "ACA HCC Score", "RAF Score", "RAF Score(Blended)", "RAF Score (Version 24)", "RAF Score (Version 28)", "RAF Score (ESRD Model)"]
    ...
    #     for hcc_measures in hcctable:
    #         try:
    #             clinical_text = hcc_measures.find_element(By.CLASS_NAME, "hcc_details event_toggle").find_elements_by_tag_name("span")[1].text
    ...
    try:
        ...
        traceback.print_exc()
        print(e)
    onlyfiles = [f for f in listdir("assets\\Download\\CSV_Files") if isfile(join("assets\\Download\\CSV_Files", f))]
    path = "assets\\Download\\CSV_Files\\" + onlyfiles[0]
...
def NameURLextractor(driver, LOB, checklist, URL_name_list, num_den_list, customer):
    ...
    for i in HCC_measure_checklist:
        try:
            if i == 400:
                Expander(driver)
                [el1, el2, el3] = driver.find_elements(By.XPATH, "//*[@id='400']//*[@class='qt-metric']//a")
                [nd1, nd2, nd3] = driver.find_elements(By.XPATH, "//*[@id='400']//*[@class='num-den']")
                time.sleep(2)
                [m1, m2, m3] = driver.find_elements(By.XPATH, "//*[@id='400']//*[@class='data-container']//*[@class='met-name top']")
                url1 = el1.get_attribute('href')
                url2 = el2.get_attribute('href')
                url3 = el3.get_attribute('href')
                print(url1, url2, url3)
                m1 = m1.text
                m2 = m2.text
                m3 = m3.text
                n1 = "Condition Management | " + m1
                n2 = "Condition Management | " + m2
                n3 = "Condition Management | " + m3
                print(n1,n2,n3)
                ...
                URL_name_list.append(LOB)
                URL_name_list.append(url3)
                URL_name_list.append(n3)
                URL_name_list.append(LOB)
            elif i == 558:
                if customer == "U.S. Renal Care":
                    Expander(driver)
                    [el1, el2, el3, el4] = driver.find_elements(By.XPATH, "//*[@id='558']//*[@class='qt-metric']//a")
                    [nd1, nd2, nd3, nd4] = driver.find_elements(By.XPATH, "//*[@id='558']//*[@class='num-den']")
                    [m1, m2, m3, m4] = driver.find_elements(By.XPATH, "//*[@id='558']//*[@class='data-container']")
                    url1 = el1.get_attribute('href')
                    print(url1)
                    url2 = el2.get_attribute('href')
                    url3 = el3.get_attribute('href')
                    url4 = el4.get_attribute('href')
                    m1 = m1.find_element(By.CLASS_NAME, "met-name top").text
                    m2 = m2.find_element(By.CLASS_NAME, "met-name top").text
                    m3 = m3.find_element(By.CLASS_NAME, "met-name top").text
                    m4 = m4.find_element(By.CLASS_NAME, "met-name top").text
                    n1 = "Condition Management | " + m1
                    n2 = "Condition Management | " + m2
                    n3 = "Condition Management | " + m3
                    n4 = "Condition Management | " + m4
                    URL_name_list.append(url1)
                    URL_name_list.append(n1)
                    URL_name_list.append(LOB)
                    URL_name_list.append(url2)
                    ...
                    URL_name_list.append(n3)
                    URL_name_list.append(LOB)
                    URL_name_list.append(url4)
                    URL_name_list.append(n4)
                    URL_name_list.append(LOB)
                else:
                    Expander(driver)
                    [el1, el2, el3] = driver.find_elements(By.XPATH, "//*[@id='558']//*[@class='qt-metric']")
                    [nd1, nd2, nd3] = driver.find_elements(By.XPATH, "//*[@id='558']//*[@class='num-den']")
                    [d1, m1, d2, m2, d3, m3] = driver.find_elements(By.XPATH, "//*[@id='558']//*[@class='data-container']")
                    url1 = el1.find_element(By.TAG_NAME, 'a').get_attribute('href')
                    url2 = el2.find_element(By.TAG_NAME, 'a').get_attribute('href')
                    url3 = el3.find_element(By.TAG_NAME, 'a').get_attribute('href')
                    d1 = d1.text
                    m1 = m1.find_element(By.CLASS_NAME, "met-name top").text
                    d2 = d2.text
                    m2 = m2.find_element(By.CLASS_NAME, "met-name top").text
                ...
                print("Already collected, skipping loop")
            else:
                time.sleep(2)
...
def display_information():
    information_text = "Please read through this information box before selecting measures:\n"\
                       ...
                       "\n" \
                       "- RCC V28 will not be visible for LA Care, HealthNet, Mollina, USRC and all OPTUM-MW CODs.\n" \
                       "\n" \
...
items_with_ids = {
    ...
    439: "Review of Chronic Conditions (Risk Adjustment Version 28)",
...
blended_dl_path = os.path.join(os.getcwd(), "assets\\Download\\CSV_Files")
...
# options.add_argument("user-data-dir=C:\\Users\\sbasu\\AppData\\Local\\Google\\Chrome\\User Data\\DefaultAutomation34")
...
for i in range(0, len(URL_name_registry_list), 3):
    ...
    try:
        ...
        search_var = measure.split(' | ')[1]
        print(search_var)
        if previous_measure != LOB + " " + measure and customer_name not in customer_list:
